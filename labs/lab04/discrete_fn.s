...
f:
    ...
    sw s0, 0(sp)
    # YOUR CODE GOES HERE!
    addi s0, a0, 3					#Hint : the args is [-3,3],if you add 3,become[0,6],
    slli s0, s0, 2					#fortunately you find it is corresponding to the array index,perfectly solving this problem
	add s0, s0, a1
    lw a0, 0(s0)
    lw s0, 0(sp)
    addi sp, sp, 4
